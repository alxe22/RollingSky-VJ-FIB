      ...
      if(!started) {
        ...
        if(dead) {
          ...
          if(Input.GetKey("space")) {
            ...
            if(checkpoint == 1) transform.position = new Vector3(0f,0.375f,-104f);
            if(checkpoint == 2) transform.position = new Vector3(0f,0.375f,-210f);
            if(checkpoint == 3) transform.position = new Vector3(0f,0.375f,-316f);
            dead = false;
        ...
        if(transform.position.z < -425) {
          ...
          if(Input.GetKey("space")) {
            UnityEngine.SceneManagement.SceneManager.LoadScene("Scene_02", LoadSceneMode.Single);
          }
        }
        //fall condition
            ...
            if(!(Input.GetKey("p") && GOD)) {
              ...
              if (isJumping || isFalling) {
                  ...
                  if (isJumping && !isFalling && transform.position.y < maxJumpHeight) {
                      transform.position = new Vector3(transform.position.x, transform.position.y + 12f*Time.deltaTime, transform.position.z);
                  }
